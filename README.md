...
Hunch always prefers the most direct layer — faster, more reliable, and (except the last) it never
touches your screen:

| Layer | Tools | What it's for |
|---|---|---|
| **OS-API** | `trash` `file_op` `open_file` `clipboard_*` `launch_app` … | files, clipboard, app lifecycle — direct API calls |
...
macOS trust attaches to the **app that runs the server** — your MCP host (Claude Desktop, Cursor,
your terminal), not to "hunch". `hunch setup` walks you through it:
